...
function createEchoMetrics(opts) {
  ...
  if (seedEnabled) maybeSeedBaseline();
  ...
  function getMetricsResponse() {
    ...
    for (const metric of Object.keys(METRICS)) {
      out[metric] = sorted.map((b) => {
        const v = perBucketValue(b, metric);
        if (v == null) return null;
        const item = { t: b.start, v, n: b.total };
        if (b.lat && b.lat[metric] && b.lat[metric].length) {
          item.p25 = percentile(b.lat[metric], 0.25);
          item.p75 = percentile(b.lat[metric], 0.75);
        }
        return item;
      }).filter((p) => p != null);
    }
